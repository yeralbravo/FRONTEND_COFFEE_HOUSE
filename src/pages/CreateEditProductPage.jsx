    ...
    useEffect(() => {
        ...
        productService
            ...
            if (product.caracteristicas && typeof product.caracteristicas === 'object') {
                const caracteristicasArray = Object.entries(product.caracteristicas).map(
                ([key, value]) => ({ key, value })
                );
                if (caracteristicasArray.length > 0) setCaracteristicas(caracteristicasArray);
            }
            if (product.images) {
                // ✅ ahora ya vienen URLs completas desde el backend
    ...
    return (
        ...
        <form onSubmit={handleSubmit} className="product-form-layout">
            ...
            <div className="form-grid">
                ...
                value={formData.tipo}
                onChange={handleChange}
                placeholder="Tipo de café (ej. Grano, Molido) *"
                required
                />
            ...
            <div className="caracteristicas-container">
                ...
                <div key={index} className="caracteristica-item">
                    ...
                    value={item.key}
                    onChange={(e) =>
                        handleCaracteristicaChange(index, 'key', e.target.value)
                    }
                    placeholder="Característica (ej. Origen)"
                    />
                    <input
                    className="value"
                    value={item.value}
