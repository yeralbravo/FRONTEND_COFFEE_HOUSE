    // src/pages/CreateEditItemPage.jsx
    ...
    useEffect(() => {
        ...
        const fetchItem = isProduct
            ? productService.getProductById(itemId)
            : insumoService.getInsumoById(itemId);
        ...
        fetchItem
            ...
            if (item.caracteristicas && typeof item.caracteristicas === 'object') {
                const caracteristicasArray = Object.entries(item.caracteristicas).map(([key, value]) => ({
                key,
                value,
                }));
    ...
    const handleSubmit = async (e) => {
        ...
        finalFormData.append(isProduct ? 'product_images' : 'insumo_images', file)
        );

        try {
        if (isEditing) {
            const updatePromise = isProduct
            ? productService.updateProduct(itemId, finalFormData)
            : insumoService.updateInsumo(itemId, finalFormData);
            await updatePromise;
            showSuccessAlert('Ítem actualizado con éxito.');
        } else {
            const createPromise = isProduct
            ? productService.createProduct(finalFormData)
            : insumoService.createInsumo(finalFormData);
            await createPromise;
    ...
    return (
        ...
        {!isEditing && (
            ...
            <button className={isProduct ? 'active' : ''} onClick={() => setItemType('product')}>
                Café
            </button>
            <button className={!isProduct ? 'active' : ''} onClick={() => setItemType('insumo')}>
                Insumo
            </button>
            </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="item-form-layout">
            ...
            <div className="form-grid">
                ...
                <>
                    ...
                    value={formData.tipo || ''}
                    onChange={handleChange}
                    placeholder="Tipo de café (ej. Grano) *"
                    required
                    />
                    <input
                    name="peso_neto"
                ...
                <input
                    ...
                    onChange={handleChange}
                    placeholder="Categoría (ej. Cafeteras) *"
                    required
                />
                )}
            ...
            <div className="caracteristicas-container">
                ...
                <div key={index} className="caracteristica-item">
                    ...
                    placeholder="Valor"
                    />
                    <button type="button" onClick={() => removeCaracteristica(index)}>
                    <FiTrash2 />
                    </button>
                </div>
                ))}
            ...
            <button
                ...
                onClick={() => navigate(isProduct ? '/supplier/products' : '/supplier/insumos')}
            >
                Cancelar
            </button>
